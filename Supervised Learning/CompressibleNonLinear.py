...
def BoundaryConditionsLinear(x,y,n_points,n_R,alpha,Dxx,Dyy,M):
    ...
    BCC_wall[int(n_points/2) + 2, int(n_points/2) + 0 + 0*n_points] = 1
    BCC_wall[int(n_points/2) + 2, int(n_points/2) + 1 + 0*n_points] = -1
    ...
    for j in range(n_points-1):
        if j < n_points - 1:
            ...
            if j % 2 != 0:
                ...
                BCC_ext[j,j + (n_R - 1)*n_points] = (-1 + x1/x2)/(y1-y2*x1/x2)
                BCC_ext[j,j + (n_R - 2)*n_points] =  1/(y1-y2*x1/x2)
                BCC_ext[j,j + 1 + (n_R - 1)*n_points] = -x1/((y1-y2*x1/x2)*x2)
    ...
    for j in range(n_points-1):
        if j % 2 != 0:
            ...
            b[j + (n_R - 1)*n_points] = sin(alpha)

    return X, Dx_y, b

def BoundaryConditionsNonLinear(X,x,y,n_points,n_R,alpha,Dx,Dy,Dxx,Dyy,Dxy,M,gamma):
    ...
    o = np.ones((n_R*n_points), dtype=np.double)
    a =  o + ((gamma-1)/2)*(M**2)*(o - (np.multiply(np.dot(Dx,X),np.dot(Dx,X) + np.multiply(np.dot(Dy,X),np.dot(Dy,X)))))
    A = a-(M**2)*np.multiply(np.dot(Dx,X),np.dot(Dx,X))
    B = a-(M**2)*np.multiply(np.dot(Dy,X),np.dot(Dy,X))
    C = - 2*(M**2)*np.multiply(np.dot(Dx,X),np.dot(Dy,X))

    Dx_y = np.zeros((n_R*n_points,n_R*n_points), dtype=np.double)

    for i in range(n_R*n_points):
        Dx_y[i,:] = A[i]*Dxx[i,:] + B[i]*Dyy[i,:] +  C[i]*Dxy[i,:]
...
def InitialValue(x,y,n_points,n_R,alpha,Dxx,Dyy,M):

    X, Dx_y, b = BoundaryConditionsLinear(x,y,n_points,n_R,alpha,Dxx,Dyy,M)
...
def DiferentialEquation(x,y,n_points,n_R,alpha,delta,lamda,n,M,gamma):
    ...
    #Linear solution
    X0 = InitialValue(x,y,n_points,n_R,alpha,Dxx,Dyy,M)


    #Non Linear Solution
    Dx_y_red_n = np.zeros((n_R*n_points,n_R*n_points - 1),dtype=np.double)
    Dx_y_redn = np.zeros((n_R*n_points-1,n_R*n_points - 1),dtype=np.double)
    X_redn = np.zeros((n_R*n_points - 1),dtype=np.double)
    b_redn = np.zeros((n_R*n_points - 1),dtype=np.double)
    ...
    for i in range(maxiter):
        ...
        aux = X_new
        X,Dx_yn, bn = BoundaryConditionsNonLinear(X_new,x,y,n_points,n_R,alpha,Dx,Dy,Dxx,Dyy,Dxy,M,gamma)
        #Imposing Dirichlet Boundary Condition
        Dx_y_red_n = Dx_yn[:,1:]
        Dx_y_redn = Dx_y_red_n[1:,:]
        b_redn =  bn[1:]

        # _, inds = sympy.Matrix(Dx_y).T.rref()   # to check the rows you need to transpose!
        # print(inds)

        X_redn = solve(Dx_y_redn, b_redn)
        X[1:] = X_redn


        # X = solve(Dx_yn,bn)
        X_new = aux + rate*(X-aux)

        print("Error iter",i,":",np.linalg.norm((np.dot(Dx,X)+np.dot(Dy,X)-np.dot(Dx,aux)-np.dot(Dy,aux)))/(n_R*n_points)**(1/2))   #np.dot(Dx,X)+np.dot(Dy,X)-np.dot(Dx,aux)-np.dot(Dy,aux)
    ...
    ax.set_title("Non Linear Compressible potential")
    plt.show()
    ...
    fig.add_subplot(211)
    plt.title('Non Linear potential contour lines')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.gca().set_aspect('equal', adjustable='box')
    levels = np.arange(-2, 2, 0.01)
    plt.contour(x, y, X_B, levels , cmap='gist_gray')
    plt.colorbar()
    plt.show()
