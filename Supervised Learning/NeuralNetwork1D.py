...
def NN(X, Y, L,theta,theta_end):
    ...
    X_extended = np.ones((rows_X+1), dtype=np.double)                 # Extended X including b terms
    X_extended[1:] = X   
                                                     
    a = np.zeros((rows_X+1,L+1), dtype=np.double)                       # a terms does not include b terms, added into z terms
    a_end = np.zeros((rows_Y), dtype=np.double)                         # exit a does also not have b terms
    z = np.zeros((rows_X,L+1), dtype=np.double)                         # including b terms
    ...
    for i in range(0,L+1):
        if i == 0:
            ...
            temp = g(z[:,i])
            a[:,i] = np.ones((rows_X+1), dtype=np.double)           # Activation function RELU   
            a[1:,i] = temp                      
...
def g(x):
    rows_X = np.size(x)
    zero = np.zeros((rows_X), dtype=np.double)
    #g = np.maximum(zero,x)
    g = np.zeros((rows_X), dtype=np.double)
...
def dg(x):
    ...
    dg = np.zeros((rows_X), dtype=np.double)
...
def Jacobian(X, Y, L,theta,theta_end):
    ...
    delta = np.zeros((rows_X,L+1), dtype=np.double)           #Not using extended X
    ...
    for l in range(0,L):                                            #For L+1 we use triang_end
        for i in range(0,rows_X):
            for j in range(0,rows_X + 1):
                triang[i,j,l] = triang[i,j,l] + a[j,l]*delta[i,l+1]

    #Now with last layer
    for i in range(0,rows_Y):
        for j in range(0,rows_X + 1):
                triang_end[i,j] = triang_end[i,j] + a[j,L]*deltaLmas1[i]

    J = triang                                           #Average for all data
    J_end = triang_end 
