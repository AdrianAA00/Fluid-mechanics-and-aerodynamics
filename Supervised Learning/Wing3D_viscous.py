...
def Geometry_wing(c1,c2,c3,SW,Kurv,b,m_change,m_panels,n_points,E,C,alpha,SW2):
    ...
    #     x[i] = (b/(2*m_panels))*i*tan(SW) + Kurv*((b/(2*m_panels))*i)**2

    #     z[i] = ((i/m_panels)**50)*1 + ((i/m_panels)**1.4)*1
    ...
    for i in range(m_panels+1):
        ...
        if i <= m_change:
            ...
            x[i] = (b/(2*m_panels))*m_change*tan(SW) + Kurv*((b/(2*m_panels))*i)**2 + max((b/(2*m_panels))*(i-m_change)*tan(SW2),0)

        z[i] = ((i/m_panels)**50)*2 + ((i/m_panels)**1.4)*3
...
def Vortex3D(alpha, delta, lamda, n, n_points, m_panels,X_3D,Y_3D,Z_3D_i,Z_3D_e,x,y,z,C,E,SW,b):
    ...
    cd_0 =  np.zeros((m_panels), dtype=np.double)
    c = np.zeros((m_panels), dtype=np.double)
    ...
    alpha = atan(sin(alpha)/(cos(alpha)*cos(SW)))
    ...
    v_ = (sin(alpha)**2 + (cos(alpha)*cos(SW))**2)**(1/2)
    ...
    for iter in range(maxiter):
        ...
        for i in range(m_panels):
            c[i] = (((X_3D[i+1,n_points] - X_3D[i+1,0]) + (X_3D[i,n_points] - X_3D[i,0]))/2)                                                                                             #Interpolate value of chord in i panel
            ...
            rot[i] = NN_cl(alpha_[i]*180/pi)*c[i]/2*v_**2
            cd_0[i] = NN_cd(alpha_[i]*180/pi)*v_**2
    ...
    plt.xlabel('Wing Span 2y/b')
    plt.ylabel('Section Cl')
    plt.plot(2*y[:m_panels]/b, Cp_global/(2*c))
    plt.show()
    ...
    # Cd_3D_vector_viscous =  np.zeros((m_panels,3), dtype=np.double)
    ...
    #     ax.quiver(X_3D[i,:n_points],-aux,Z_3D_i[i,:n_points],Cp_3D_vector[i,:,0],Cp_3D_vector[i,:,1],Cp_3D_vector[i,:,2],length=10) 
    #     ax.quiver(X_3D[i,n_points],aux[0],Z_3D_i[i,n_points],Cd_3D_vector[i,0],Cd_3D_vector[i,1],Cd_3D_vector[i,2],color="r",length=500) 
    #     ax.quiver(X_3D[i,n_points] + Cd_3D_vector[i,0]*500,aux[0],Z_3D_i[i,n_points],Cd_3D_vector_viscous[i,0],Cd_3D_vector[i,1],Cd_3D_vector[i,2],color="b",length=500)
    #     ax.quiver(X_3D[i,n_points],-aux[0],Z_3D_i[i,n_points],Cd_3D_vector[i,0],Cd_3D_vector[i,1],Cd_3D_vector[i,2],color="r",length=500) 
    #     ax.quiver(X_3D[i,n_points] + Cd_3D_vector[i,0]*500,-aux[0],Z_3D_i[i,n_points],Cd_3D_vector_viscous[i,0],Cd_3D_vector[i,1],Cd_3D_vector[i,2],color="b",length=500)
    ...
    # print("Real alpha", alpha*180/pi)
    # print("Real velocity", v_)
...
def VelocityRepresentation(rot,n_points,m_panels,X_3D,Y_3D,Z_3D_i,Z_3D_e,x,y,z,alpha):
    ...
    # ax.set_ylim3d(-50, 50)
    # ax.set_zlim3d(-50, 50)
    # ax.quiver(xyz_[:,0],xyz_[:,1],xyz_[:,2],v_[:,0],v_[:,1],v_[:,2],length=200) 

    # # ax.plot_trisurf(X_3D.flatten(),Y_3D.flatten(),Z_3D_e.flatten(),cmap='Greys', edgecolor='none')
    # # ax.plot_trisurf(X_3D.flatten(),Y_3D.flatten(),Z_3D_i.flatten(),cmap='Greys', edgecolor='none')
    # # ax.plot_trisurf(X_3D.flatten(),-Y_3D.flatten(),Z_3D_e.flatten(),cmap='Greys', edgecolor='none')
    # # ax.plot_trisurf(X_3D.flatten(),-Y_3D.flatten(),Z_3D_i.flatten(),cmap='Greys', edgecolor='none')
    # ax.scatter(X_3D.flatten(),Y_3D.flatten(),Z_3D_e.flatten(),s=2, c='Black') 
    # ax.scatter(X_3D.flatten(),Y_3D.flatten(),Z_3D_i.flatten(),s=2,c='Black')
    # ax.scatter(X_3D.flatten(),-Y_3D.flatten(),Z_3D_e.flatten(),s=2,c='Black') 
    # ax.scatter(X_3D.flatten(),-Y_3D.flatten(),Z_3D_i.flatten(),s=2,c='Black')
    ...
    # #initial_point = np.array([(x[m_panels])-5,(y[m_panels]+0.01),(z[m_panels] + 0.5)])
    # initial_point = np.array([(x[m_panels])-5,(y[m_panels]+0.01),(z[m_panels] - 0.75)])
    # xyz_s = StreamLines(initial_point, steps, size_step, XYZ_R,XYZ_L,rot,m_panels,alpha)
    ...
    # #initial_point = np.array([(x[m_panels])-5,(y[m_panels]+0.01),(z[m_panels] - 0.5)])
    # initial_point = np.array([(x[m_panels])-5,(y[m_panels]+0.01),(z[m_panels] - 1)])
    # xyz_s = StreamLines(initial_point, steps, size_step, XYZ_R,XYZ_L,rot,m_panels,alpha)
