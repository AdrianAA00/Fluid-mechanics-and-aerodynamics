...
def BoundaryConditions(x,y,n_points,n_R,alpha,Dxx,Dyy,M):
    ...
    #Exterior boundary
    BCC_ext = np.zeros((n_points,n_R*n_points), dtype=np.double)        
    ...
    for j in range(n_points):
        ...
        if int(2*n_points/6) < j < int(4*n_points/6):
    ...
    #Exterior boundary
    for j in range(n_points):
        if j < int(n_points/6) or j >= int(5*n_points/6):
            #Dx
    ...
    for j in range(n_points):
        if j < int(n_points/4) or j >= int(3*n_points/4):
            if j % 2 != 0:
