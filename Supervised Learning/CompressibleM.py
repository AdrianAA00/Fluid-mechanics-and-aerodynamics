...
def Airfoil_net(R_max, n_R, delta, lamda, alpha, n, n_points):
    ...
    for i in range(n_R):
        ...
        x[i,:] = [ele.real for ele in A] # extract real part
        y[i,:] = [ele.imag for ele in A] # extract imaginary part

        plt.plot(x[i,:], y[i,:],c="black")
        plt.scatter(x[i,:], y[i,:],c="red", s = 20)
...
def BoundaryConditions(x,y,n_points,n_R,alpha,Dxx,Dyy,M):
...
def DiferentialEquation(x,y,n_points,n_R,alpha,delta,lamda,n,M):
    ...
    X, Dx_y, b= BoundaryConditions(x,y,n_points,n_R,alpha,Dxx,Dyy,M)
    ...
    plt.contour(x, y, X_B, levels , cmap='gist_gray',vmin = -2, vmax = 2)
    plt.colorbar()
    plt.show()
